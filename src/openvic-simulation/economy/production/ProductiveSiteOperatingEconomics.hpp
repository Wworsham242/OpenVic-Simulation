...
/// existing market bridge. The remaining components are explicitly imputed
/// operating-cost signals until dedicated wage, utility-billing, freight,
/// and firm-ledger systems settle those obligations in cash.
struct ProductiveSiteOperatingEconomicsResult final {
	...
	fixed_point_t labor_cost_proxy = fixed_point_t::_0;
...
public:
	[[nodiscard]] static ProductiveSiteOperatingEconomicsResult calculate(
		...
		fixed_point_t labor_cost_proxy,
		...
		ProductiveSiteOperatingEconomicsResult result {
			...
			.labor_cost_proxy =
				std::max(labor_cost_proxy, fixed_point_t::_0),
